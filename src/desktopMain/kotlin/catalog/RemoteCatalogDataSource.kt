...
class RemoteCatalogDataSource : CatalogDataSource {
     ...
     */
    private fun loadWithCacheAge(maxAge: Duration, forceRefresh: Boolean = false, log: (String) -> Unit = {}): Catalog? {
        try {
    ...
    private fun fetchAllCsvPages(log: (String) -> Unit): String {
        ...
        runCatching { fetchRaw(urlApi, log) }
    ...
    private fun tryCsvThenHtml(log: (String) -> Unit): Catalog? {
        ...
        val html = runCatching { fetchRaw(urlSource, log) }.onFailure { log("HTML fetch failed: ${it.message}") }.getOrNull() ?: return null
